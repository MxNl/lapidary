...
`ind_rise_rate` / `ind_fall_rate` are the median magnitudes of the
positive and negative step-to-step changes. A large rise rate with a
small fall rate is a system driven by sharp recharge pulses that then
drain slowly.

## Usage

``` r
lap_ind_rise_fall(data, value = gwl, date = "date", ...)
```
...
A one-row tibble: `ind_rise_rate`, `ind_fall_rate`.
