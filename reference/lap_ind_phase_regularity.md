...
month in which each year's minimum level falls. Near 0 = a metronomic
seasonal cycle; large = event-driven / irregular timing.

## Usage

``` r
lap_ind_phase_regularity(data, value = gwl, date = "date", ...)
```
...
A one-row tibble: `ind_min_month_sd`.
