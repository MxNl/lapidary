test_that("a well's first year is never a record", {
  df <- data.frame(
    well_id = "w1", date = as.Date(c("2020-03-01", "2020-11-01")), gwl = c(5, 1)
  )
  out <- lap_add_record_flags(df)
  expect_true(all(!out$is_new_min))
  expect_true(all(!out$is_new_max))
})

test_that("only the annual min/max timestep can be flagged, and only if it beats history", {
  df <- data.frame(
    well_id = "w1",
    date = as.Date(c(
      "2018-03-01", "2018-07-01", "2018-11-01", # year min 4, max 6 (first year)
      "2019-02-01", "2019-08-01" # year min/max both 4: ties the running values, no record
    )),
    gwl = c(5, 6, 4, 4, 4)
  )
  out <- lap_add_record_flags(df)
...
test_that("a year that beats history flags exactly its own extreme timestep", {
  df <- data.frame(
    well_id = "w1",
    date = as.Date(c(
      "2018-03-01", "2018-07-01", "2018-11-01", # 2018: min 4, max 6 (first year)
      "2019-02-01", "2019-08-01", # 2019: min 4, max 4 (ties, no record)
  ...
  expect_equal(sum(out$is_new_min), 1L)
  expect_equal(sum(out$is_new_max), 1L)
})
...
test_that("multiple wells are evaluated independently", {
  ...
  expect_true(all(!w2$is_new_min)) # all tied at 1, never beats the first year
})
...
test_that("flags are computed chronologically, independent of input row order", {
  ...
  expect_identical(out$is_new_min, c(FALSE, TRUE, TRUE))
})
...
test_that("record_balance is +1/-1 on a record row, 0 elsewhere, and sums correctly", {
  df <- data.frame(
    well_id = "w1",
    date = as.Date(c(
      "2018-03-01", "2018-07-01", "2018-11-01", # 2018: first year, no records
  ...
  out <- lap_add_record_flags(df)
  expect_identical(out$record_balance, c(0L, 0L, 0L, -1L, 1L))
  # summing record_balance per bucket equals summing is_new_max - is_new_min
  # separately (the whole point of the single-column shortcut)
