...
`"all"` is data-aware: it runs every indicator whose inputs are in the data.
Most need only the level column, but the drought and climate ones need a
standardised index (`lap_normalise_gwl("sgi")`) and, for `climate_signal`, a
joined driver (`lap_join_meteo()`). Add those columns and `"all"` picks the
indicators up by itself, using `gwl_norm` and `precip` in place of `value`; if
they are missing it says which indicators it skipped and why.
