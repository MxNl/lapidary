...
series. Pass `lap_ind_*()` functions to `lap_summarise_wells(indicators = )` for
a single pass, or use `lap_indicators()` / `lap_add_indicators()` to build a set
step by step.
...
lap_summarise_wells(
  gems_ger_sample,
  by = well_id,
  indicators = c(lap_ind_amplitude, lap_ind_extreme_months, lap_ind_trend)
) |>
