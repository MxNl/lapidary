...
The SGI is standardised over a well’s **whole** record. A window of it -
a decade in
[`lap_indicator_change()`](https://mxnl.github.io/lapidary/reference/lap_indicator_change.md) -
may sit far below zero, which is exactly what a multi-year drought is.
`lap_indicators("all")` therefore checks the index once, on the full
record, and passes `check = FALSE` down; if you slice windows yourself
and request `drought` by key, pass `check = FALSE` too, or the guard
will reject your driest series. The **SGI** (Standardised Groundwater
Index, Bloomfield & Marchant 2013) is a non-parametric normal-scores
transform applied per calendar month: within each month the values are
ranked and mapped through `qnorm(rank / (n + 1))`, giving a
deseasonalised, unit-variance index directly comparable across wells
with very different dynamic ranges. A **drought event** is a maximal run
of `SGI < threshold` (default `-1`), i.e. run theory (Yevjevich 1967).
